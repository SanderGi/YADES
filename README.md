...
[![maintenance-status](https://img.shields.io/badge/maintenance-experimental-blue.svg?style=for-the-badge)](https://gist.github.com/taiki-e/ad73eaea17e2e0372efb76ef6b38f17b)

Inference and training code to detect whether the camera of an embedded system is looking at an animal or not. The model is fine-tuned from [YOLOv8n-cls](#about-yolov8n-cls) and is optimized for low-power and low-memory use in embedded systems, specifically the [MAX78000 microcontroller](https://github.com/analogdevicesinc/MaximAI_Documentation/blob/main/Guides/MAX78000_Workflow_Guide.md). The MAX78000 has a 432 KiB (442 KB) weight limit so quantization, pruning and distillation techniques are used to reduce the model size. For comparison, a shallow 6-layer CNN is also trained with [QAT](#about-quantization-aware-training-qat). An 80-10-10 train-val-test split is applied to [this dataset](https://www.kaggle.com/datasets/tylerelvis/animal-vs-non-animal-image-recognition-dataset) scraped from Google Captcha v2 (34155 images). The dataset is well balanced between animal (18107) and non-animal (16048) images (53% animal vs 47% non-animal).

![Test Accuracy bar chart for Model Sizes below 432 KiB (442 KB)](data/test_accuracy_and_model_size_small.png)
...
![Test Accuracy and Model Size scatter plot](data/test_accuracy_and_model_size_scatter.png)

For baselining purposes, we also evaluated the pretrained YOLOv8n-cls model's performance (84% test accuracy, 5.8 MB). A similarly-sized (5.4 MB) 6-layer CNN model achieves 90% accuracy on the test set. A simple fine-tuned YOLOv8 model achieves 99.1% accuracy on the test set.
...
- Use `python ./scripts/MAX78000/gen_simple_cnn.py` to generate custom CNNs for the MAX78000. You can either pass a config file `-config data/test-configs/4_4_4_4_32_32_32.txt`, configure it interactively `--interactive`, or set a layer and size target and have the script figure things out `-l 6 -s 10000`.
...
- Use `. scripts/MAX78000/train.sh` to train the YOLOv8p model optimized for the MAX78000
    ...
    - Make sure you have first run `notebooks/EDA.ipynb` to format the training data.
    - For best results, comment out line 14 and un-comment line 13 in `scripts/MAX78000/train.sh`. For fastest training time, instead make a copy of `MAX78000/ai8x-training/policies/qat_policy.yaml`, name it `qat_policy_fast.yaml`, and change the `start_epoch` to 3.
...
- Use `. scripts/MAX78000/eval.sh <checkpoint>` to evaluate the YOLOv8p model on the MAX78000
    ...
    - Run `cp MAX78000/ai8x-training/sample_animal_detection.npy data/sample_animal_detection.npy` to save the sample data for synthesis.
    - You can skip if you just want to trust me bro.
- Use `. scripts/MAX78000/train.sh --yaml-template ../../scripts/MAX78000/yolo-pico.yaml` to generate the yaml template for the model
    - Then set `streaming: true` on the first two layers and manually edit the data memory allocation.
    - You can skip this and just use the premade `scripts/MAX78000/yolo-pico.yaml`.
...
- Use the MSDK to build, flash, and run the model on the MAX78000.
    ...
    - See [the documentation](https://github.com/analogdevicesinc/ai8x-synthesis?tab=readme-ov-file#starting-an-inference-waiting-for-completion-multiple-inferences-in-sequence) for modifying what the sample code does. 
...
- Setup sdk
    ...
    - `mkdir MAX78000/ai8x-synthesis/sdk`
...
│   ├── animal_cnn_224_small.pt                    # best 6-layer CNN <432 KiB
...
### About YOLOv8p
The smallest officially available YOLOv8 checkpoint is "nano" (see [YOLOv8n](#about-yolov8n-cls)) which is 5.8 MB. We can scale down the layers and channels to fit within the 432 KiB limit of the MAX78000. I have named this YOLOv8p for "pico" and the architecture can be found in [scripts/yolo_pico.py](scripts/yolo_pico.py).
